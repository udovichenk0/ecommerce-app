...
const selectParent = (state: Record<typeof pageName, Page>) => state['page/shop']
const selectEntityProduct = createSelector(selectParent, (state) => state['entity/products'])
const $$products = createProducts(selectEntityProduct);

const shopReducers = combineReducers({
  [$$products.name]: $$products.reducer
})
const getProductsFx = createAsyncThunk(pageName + 'fetch', async (_, { dispatch, getState }) => {
...
const getNextProductFx = createAsyncThunk(pageName + 'next-products', async (_, { dispatch, getState }) => {
  const state = getState() as Record<typeof pageName, Page>
  const lastRefKey = $$products.selectors.lastRefKey(state)
