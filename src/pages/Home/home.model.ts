...
import { CreateProducts, ProductsState, createProducts } from "@/entities/products"
import { productApi } from "@/shared/api/product"

const pageName = 'page/home'
type Page = Record<CreateProducts['name'], ProductsState>

const selectParent = (state: Record<typeof pageName, Page>) => state['page/home']

const featurePrefix = 'featured'
const selectFeatureProducts = createSelector(selectParent, (state) => state[featurePrefix + "/entity/products"])
const $$featuredProducts = createProducts(selectFeatureProducts, featurePrefix)

const recommendedPrefix = 'recommended'
const selectRecommendedProducts = createSelector(selectParent, (state) => state[recommendedPrefix + "/entity/products"])
...
const getFeaturedProductsFx = createAsyncThunk(pageName + 'featured-products', async (_, {dispatch}) => {
